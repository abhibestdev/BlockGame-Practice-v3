...
import us.blockgame.practice.queue.menu.button.ranked.QueueRankedKitButton;
...
public class QueueRankedMenu extends Menu {

    @Override
    public String getTitle(Player player) {
        return ChatColor.BLUE.toString() + ChatColor.BOLD + "Join a Ranked Queue";
    }
    ...
    public Map<Integer, Button> getButtons(Player player) {
        ...
        Arrays.stream(Kit.values()).forEach(k -> {
            buttonMap.put(atomicSlot.getAndAdd(1), new QueueRankedKitButton(k));
        });
